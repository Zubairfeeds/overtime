...
function fetchAttendanceData(frm) {
	...
	frappe.call({
		method: 'overtime.overtime.doctype.overtime.overtime.get_attendance_data',
		args: {
			employee: employee,
			from_date: from_date,
			to_date: to_date,
...
function fetchStandardWorkingHours(frm, row) {
	frappe.call({
		method: 'overtime.overtime.doctype.overtime.overtime.get_standard_working_hours',
		callback: function (response) {
...
function gross_monthly_salary(frm) {
	...
	frappe.call({
		method: 'overtime.overtime.doctype.overtime.overtime.get_gross_salary',
		args: {
			employee: employee
		},
		callback: function (response) {
	...
	refresh: function (frm) {
		...
		if (frm.doc.docstatus == 1 && frm.doc.repay_from_salary == 0) {
			frm.add_custom_button(__('Create Expense Claim'), function () {
				frappe.call({
					method: 'overtime.overtime.doctype.overtime.overtime.make_expense_claim',
					args: {
